...
import get_student_deploy

#　教師データ初期化
data_teacher = ['絹川', '小山', '矢島', '齊藤', '小坂', '中島', '高橋', '鉄谷', '川澄', '増田',
	'猪俣・佐々木', '岩井', '竜田', '山田', '柿崎', '森谷', '井ノ上', '学科外(系列等)', '(未定)']
data_teacher_anonymaus = ['knkw', 'kym', 'yjm', 'situ', 'osk', 'nkjm', 'tkhs', 'tttn', 'kwsm', 'msd',
	'inmt\nssk', 'iwi', 'ttt', 'ymd', 'kkzk', 'mry', 'inue', '学科外(系列等)', '(未定)']
X_teacher = list(data_teacher_anonymaus)
# X_teacher[10] = '猪俣\n佐々木'
X_teacher[17] = '学科外\n(系列等)'
...
def parseHtml(prev_list, new_list):
	for i, line in enumerate(prev_list):
		if line.find("fi")!=-1:
			new_list.append(str(i)+line.replace('	','').replace('<tr><td>',',').replace('</td><td>',',').replace('</td></tr>',''))
...
def drawGraph(prev_list, now_list, diff_list, incr_list, decr_list):
	...
	# figname1 = "/Users/ryosuke/Desktop/student-deploy-log/"
	figname1 = "./"
	figname2 = ".png"

	#　タイトル（日時）の作成
	title = "=== 2016_Student_Deploy_Graph : " + figday + "(" + figtime + ") ==="
	savename = figname1 + "Anonymaus " + figday + "(" + figtime + ")" + figname2
	plt.title(title)
...
def main():
	...
	student_deploy_list = []
	parseHtml(getPrevLog('log_student_deploy.txt'), prev_student_deploy_list)
	...
	if flag == True:
		...
		setGraphColor(now_Y)
		#　グラフ描画関数
		drawGraph(prev_Y, now_Y, diff_Y, incr_Y, decr_Y)
	else:
